...
namespace termexplorer.FileTransfer
{
    class Downloader
    {
        ...
        {
            // Comform
            bool check1 = BoxWriter.CheckScreen("Really Receive File?", $"This progress can not stop.", false);
            if (check1 == false) return;

            BoxWriter.Splash("Waiting for Uploader");
            byte[] recvdata = Download(2222);

            string UserHash = BoxWriter.AskToUserScreen("File Transfer", "Accual Hash");
            if (Shared.GetHash(recvdata) != UserHash)
            {
                BoxWriter.PopScreen("File broken", $"Received file is broken");
                return;
            }

            string sfname = BoxWriter.AskToUserScreen("File Transfer", "Put FileName (can't overwrite it)");
            string fname = System.IO.Path.Combine(basedir.FullPath,sfname);
            if (System.IO.File.Exists(fname))
            {
                BoxWriter.PopScreen("FileName already used", $"Filename already used");
                return;
            }

            // Comform
            bool check2 = BoxWriter.CheckScreen("Really Receive This?", $"The file will save to \"{fname}\"", false);
            ...
            BoxWriter.PopScreen("Success", $"The file was saved to \"{fname}\"");
        }
    }
}
